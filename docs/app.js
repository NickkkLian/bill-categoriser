...
function viewDuplicates(main, m) {
  ...
  const pair = (a, b, label, note, acts) => h('div', { class: 'card', style: 'margin-bottom:8px' }, h('div', { class: 'grid-2' }, [['retained', a], [label, b]].map(([t, r]) => h('div', {}, h('div', { class: 'faint', style: 'font-size:var(--text-2xs);text-transform:uppercase;letter-spacing:.04em' }, t), r ? h('div', { style: 'margin-top:4px' }, srcChip(r.id), ' ', h('span', { class: 'mono' }, r.invoice), ' · ', h('span', { class: 'mono' }, r.date || '—'), ' · ', r.merchant, ' · ', h('span', { class: 'mono' }, money(r.cents))) : h('div', { class: 'muted' }, '(source row not found)')))), h('p', { class: 'hint', style: 'margin-top:8px;max-width:var(--measure)' }, note), acts ? h('div', { class: 'toolbar', style: 'margin-top:8px' }, acts) : null);
