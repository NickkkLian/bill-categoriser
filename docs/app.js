...
async function doImport(file) {
  ...
  const loading = h('div', { class: 'card loading', role: 'status' }, h('h3', {}, `Reading ${file.name}…`), h('span', { class: 'skel', style: 'width:62%' }), h('span', { class: 'skel', style: 'width:38%' }));
  ...
  loading.remove();
  const main = $('#main'); main.prepend(h('div', { class: 'error', role: 'alert' }, h('h3', {}, `Couldn't read ${file.name}`),
...
function viewBills(main, m, p) {
  ...
  const tbl = h('table', { class: 'data' }, h('thead', {}, h('tr', {}, thSort('Src', 'src', p, 'sticky'), thSort('Invoice', 'invoice', p), thSort('Date', 'date', p), thSort('Merchant', 'merchant', p), thSort('Category', 'category', p), thSort('Amount (CAD)', 'amount', p, 'num'), h('th', { scope: 'col' }, 'Flags'), h('th', { scope: 'col' }, h('span', { class: 'sr-only' }, 'Actions')))),
    h('tbody', {}, view.length ? view.map(r => h('tr', { tabindex: '0', 'aria-selected': r.id === sel ? 'true' : null, dataset: { id: r.id }, onclick: () => setParam('i', r.id), onkeydown: e => { if (e.key === 'Enter') setParam('i', r.id); } },
      ...
      : h('tr', {}, h('td', { colspan: 8, class: 'empty-row' }, 'No rows match. ', h('button', { class: 'btn btn-sm', onclick: () => go('bills') }, 'Clear filters')))));
...
function viewRules(main, m) {
  ...
  const edit = (row) => { const sel = h('select', {}, [...CATEGORIES, 'Uncategorised'].map(c => h('option', { value: c, selected: c === row.effective }, c))); return h('span', { class: 'rule-edit' }, sel, h('button', { class: 'btn btn-sm', onclick: () => { const cat = sel.value; const np = { ...pend, aliases: pend.aliases.filter(x => x.alias !== row.alias), dirty: true }; if (!(row.who === 'build' && cat === row.category)) np.aliases.push({ id: uid(), alias: row.alias, category: cat, who: 'you', at: nowIso() }); S.pending = np; render(); } }, 'Set')); };
  ...
  const newAlias = h('input', { type: 'text', placeholder: 'merchant name as it appears after normalisation', style: 'min-width:260px' }), newCat = h('select', {}, CATEGORIES.map(c => h('option', { value: c }, c)));
...
function chartCard(title, note, data, tableCols, key, opts) {
  ...
  return h('div', { class: 'chart' }, h('div', { class: 'ch-head' }, h('h3', {}, title), seg, h('p', {}, note)), body);
}
...
function viewLog(main, m, p) {
  ...
  main.append(h('div', { class: 'tablewrap' }, h('table', { class: 'data' }, h('thead', {}, h('tr', {}, thSort('#', 'n', p, null, 'sort', 'n:asc'), thSort('Src', 'src', p, null, 'sort', 'n:asc'), thSort('Field', 'field', p, null, 'sort', 'n:asc'), h('th', { scope: 'col' }, 'Before → after'), thSort('Rule', 'rule', p, null, 'sort', 'n:asc'), thSort('Who', 'who', p, null, 'sort', 'n:asc'), thSort('When', 'at', p, null, 'sort', 'n:asc'), h('th', { scope: 'col' }, h('span', { class: 'sr-only' }, 'Actions')))),
    h('tbody', {}, view.map(c => h('tr', { onclick: () => go('bills', { i: c.id, excluded: '1' }) }, h('td', { class: 'mono' }, c.n), h('td', {}, srcChip(c.id, false)), h('td', {}, c.field), h('td', { class: 'mono', title: JSON.stringify(c.before) + ' → ' + JSON.stringify(c.after) }, JSON.stringify(c.before), ' → ', h('b', {}, JSON.stringify(c.after))), h('td', {}, c.rule), h('td', {}, c.who), h('td', { class: 'mono' }, c.at ? hhmm(c.at) : '—'), h('td', { class: 'actions' }, c.who === 'you' && c.did && !c.rule.startsWith('reviewer undo') && !S.decisions.find(d => d.id === c.did)?.undoneBy ? h('button', { class: 'btn btn-ghost btn-sm', onclick: e => { e.stopPropagation(); undoDecision(c.did); toast('Decision undone'); render(); } }, 'undo') : null)))))));
